...
int main() {
	...
	//									 manipula��o de dados j� armazenados
	...
	// matriz com todos os nomes dos albuns na coluna 1 e sua classifica��o na coluna 2, artista na coluna 3
    ...
    if(((vet_estrelas[0] != vet_estrelas[1]) && (vet_estrelas[0] != vet_estrelas[2]) && (vet_estrelas[1] != vet_estrelas[2])) || (vet_estrelas[0]==3 && vet_estrelas[1]==4 && vet_estrelas[2]==4)){
	for (int i = 0; i < 3; i++) { // ordenar o vet_estrela e fazer os mesmos swaps com o vet_linha
		for (int j = 0; j < 3; j++) {
			if(vet_estrelas[i] > vet_estrelas[j]) { // swap
